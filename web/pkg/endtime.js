/* @ts-self-types="./endtime.d.ts" */

/**
 * @param {string} sep
 * @param {number} dozenals
 * @returns {string}
 */
export function endtime_now_string(sep, dozenals) {
    let deferred2_0;
    let deferred2_1;
    try {
        const ptr0 = passStringToWasm0(sep, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
        const len0 = WASM_VECTOR_LEN;
        const ret = wasm.endtime_now_string(ptr0, len0, dozenals);
        deferred2_0 = ret[0];
        deferred2_1 = ret[1];
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('endtime_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
